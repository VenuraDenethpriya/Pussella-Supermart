...
if(isset($_POST['update_product'])){
   ...
   $update_product = $conn->prepare("UPDATE `products` SET name = ?, category = ?, mweight = ?, weight = ?, details = ?, price = ? WHERE id = ?");
   $update_product->execute([$name, $category, $mweight, $weight, $details, $price, $pid]);
...
<section class="update-product">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <option selected><?= $fetch_products['mweight']; ?></option>
               <option value="kg">kg</option>
               <option value="g">g</option>
               <option value="nun"> </option> 
         </select>
         <input type="text" min="0" name="weight" class="box" required placeholder="Enter net weight" value="<?= $fetch_products['weight']; ?>">
